...
// 自定义，印象笔记客户端
const eversyncclient_1 = require("../eversyncclient");
...
let sf = timeutil_1.default.TimeStampToString(timestamp);
console.log(sf, path);
print();
// -------------- 读文件
//
...
// 转换工具类
let converter = new enmlconverter_1.default();
// let 定义局部变量
// 调用客户端
let client = new eversyncclient_1.default();
...
client.syncAccount(config.token, config.noteStoreUrl).then(function (result) {
    ...
    client.getNoteGuidByTitle(title).then(function (guid) {
        ...
        client.getNoteContent(noteGuid).then(function (note) {
            console.log("--guid:\n", note.guid);
            console.log("--标题:\n", note.title);
            console.log("--创建时间:\n", timeutil_1.default.TimeStampToString(note.created));
            console.log("--更新时间:\n", timeutil_1.default.TimeStampToString(note.updated));
            console.log("--文件大小:\n", note.contentLength);
            console.log("--笔记本guid:\n", note.notebookGuid);
            console.log("--内容:\n", note.content);
            console.log("---> 笔记本内容:\n%s\n", note.content);
